...
<script>
    ...
    const {
        ...
        el: addCommentModalElement,
        close: addCommentModalClose
    } = renderModal({
        ...
        onSubmit: (fd) => {
            ...
            console.log("here56");
            $.ajax({ // запрос на комментирование
    ...
    @if($isUserAdmin)
    const {
        open: blogRecordRedactorModalOpen,
        close: blogRecordRedactorModalClose
    } = renderModal({
        renderTo: document.body,
    ...
    <tbody class="posts-table">
        ...
        <tr data-record-id="{{$posts->id}}">
            ...
            <td class="post__image">
                @if ($posts->image)
                <img src="{{ $posts->image }}" alt="{{ $posts->title }}" style="max-width: 40%;">
                @endif
            </td>
            <td>
                @if($isUserAdmin)
